...
hClient.prototype.processMsgInternal = function(hMessage, cb) {
    ...
    validator.validateHMessage(hMessage, function(err, result){
        ...
        if( !validator.compareJIDs(self.jid, hMessage.publisher) )
            return cb(self.buildResult(hMessage.publisher, hMessage.msgid, 'send', codes.hResultStatus.NOT_AUTHORIZED, 'publisher does not match sender'));
            ...
            if( validator.isChannel(hMessage.actor) ){
                if(type === "hcommand"){
                    self.cmdController.execCommand(hMessage, function(result) {
                        ...
                        if ((hMessage.timeout == 0 && result.payload.status != 0) || hMessage.timeout > 0 || hMessage.timeout === undefined)
                            cb(result)
                    });
                }
                else{
                    hAdmin.publishMessage(hMessage, function(status, result) {
                        var hMessageResult = self.buildResult(hMessage.publisher, hMessage.msgid, 'send', status, result);
                        if( hMessage.persistent === true ){
                ...
                if(type === "hcommand")
                    ...
                    if (hMessage.timeout >= 0)
                        return cb(self.buildResult(hMessage.publisher, hMessage.msgid, 'send', codes.hResultStatus.NOT_AUTHORIZED, 'server only accepts hCommand payload'));
                }
            else
                self.sendMessage(hMessage);
        } catch(err) {
            try{
                return cb(self.buildResult(hMessage.publisher, hMessage.msgid, 'send', codes.hResultStatus.TECH_ERROR, 'error during sending message'));
            }
            catch(err){
                log.error(err);
            }
 ...
 * WARN : if updated here should also be update in hAdmin
 */
hClient.prototype.buildResult = function(actor, ref, cmd, status, result) {
    var hmessage = {};
