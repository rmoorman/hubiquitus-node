...
hAdmin.prototype.processMsg = function(hMessage) {
    ...
    validator.validateHMessage(hMessage, function(err, result){
        ...
        if(db.cache.hChannels[hMessage.actor])
            channel = db.cache.hChannels[hMessage.actor].payload;
        ...
        else if( validator.compareJIDs(hMessage.actor, this.xmppOptions.jid) ) //process the message if it's a command to the server
 ...
 * WARN : if updated here should also be update in hClient
 */
hAdmin.prototype.buildResult = function(ref, cmd, status, result) {
    var hmessage = {};
    ...
    hmessage.publisher = this.xmppOptions.jid;
...
hAdmin.prototype._initAdminChannel = function(cb){
    ...
    /*this.cmdController.execCommand(getChannelsCmd, this.jid, function(hMessage){
        ...
        if(hResult.status == codes.hResultStatus.OK){
            ...
            self.cmdController.execCommand(createAdminCmd, self.jid, function(hMessage){

                //When subscribed, and thus our epic story is finished...
                self.cmdController.execCommand(subscribeCmd, self.jid, function(hMessage){
                    return cb();
                })
...
hAdmin.prototype.publishHChannel = function(hChannel, cb){
    var publishMsg = this.buildMessage(this.adminChannel, 'hChannel', hChannel);

    if(this.status == codes.statuses.CONNECTED)
        self.publishMessage(hMessage, cb);
    else
        cb(this.buildResult(publishMsg.msgid, 'hpublish', codes.hResultStatus.NOT_CONNECTED, 'the user is not connected'));
};
