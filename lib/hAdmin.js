...
hAdmin.prototype.processMsg = function(hMessage) {
    ...
    if(hMessage && validator.isChannel(hMessage.actor)) {
        hMessage.actor = validator.normalizeChannel(hMessage.actor, validator.getDomainJID(this.jid));
    }
    ...
    validator.validateHMessage(hMessage, function(err, result){
        ...
        if(err)
            return self.emit("hMessage", self.buildResult(hMessage.publisher, hMessage.msgid, undefined, err, result));
        ...
        if( validator.isChannel(hMessage.actor) ) //publish it if it's a channel
            self.publishMessage(hMessage, function(status, result) {
                self.emit("hMessage", self.buildResult(hMessage.publisher, hMessage.msgid, "hpublish", status, result))
            });
        else if( validator.compareJIDs(hMessage.actor, this.jid) ) //process the message if it's a command to the server
            if(hMessage.type.toLowerCase() === "hcommand")
                self.cmdController.execCommand(hMessage, function(status, result) {
                    self.emit("hMessage", result)
                });
            else
                return self.emit("hMessage", self.buildResult(hMessage.publisher, hMessage.msgid, undefined, statuses.NOT_AUTHORIZED, 'server only accepts hCommand payload'));
        else
...
hAdmin.prototype.publishHChannel = function(hChannel, cb){
    ...
    if(this.status == codes.statuses.CONNECTED)
        this.publishMessage(publishMsg, cb);
    else
        cb(this.buildResult(publishMsg.publisher, publishMsg.msgid, 'hpublish', codes.hResultStatus.NOT_CONNECTED, 'the user is not connected'));
};
