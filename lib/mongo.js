 ...
 * @private
 */
Db.prototype._saver = function(collection, doc, cb){
    if(this._testValidators(collection, doc)){
        if(cb)
            collection.save(doc, {safe: true}, cb);
        else
            collection.save(doc);

    } else if(cb)
        cb('validation error', doc);
};
