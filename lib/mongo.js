...
Db.prototype._saver = function(collection, doc, options, cb){
    ...
    var callback = function(err, result){
        ...
        if(cb){
            if(err)
                return cb(codes.hResultStatus.TECH_ERROR, JSON.stringify(err));
            else
                return cb(err, savedElem);
        }
    };
...
Db.prototype.saveHChannel = function(hChannel, cb){
    ...
    this._testValidators(hChannel, this.collections.hChannels.validators, function(err, msg){
        if(err)
