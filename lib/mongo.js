...
var Db = function(){
    ...
    this._staticCol = ['hCommands', 'hResults', 'hSubscriptions', 'hChannels', 'hMessages'];
    this.collections = {};
...
//    this.collections.hChannels.ensureIndex('_id', {unique: true});
    ...
    this.collections.hChannels.onSave = [function(doc){
        ...
        hAdmin.publishHChannel(doc);
    }];

    //Symbolically create hMessages
    this.get('VirtualHMessages');
...
Db.prototype._saver = function(collection, doc, options, cb){
    ...
    var callback = function(err, result){
        ...
        if(!err){
            ...
            return typeof cb === 'function' ? cb(codes.hResultStatus.TECH_ERROR, '' + err) : null;
        }
    };
...
Db.prototype.saveHMessage = function(hMessage, cb){
    //If the chid is that of a client, save it to a collection called hMessages, else to the channel's collection
    var collection = validators.isChannel(hMessage.actor) ? hMessage.actor : 'hMessages';

    //Use 'virtual' hMessages collection to test requirements. But when saving use real collection
    this._saver(this.get(collection), hMessage, {virtual: this.get('VirtualHMessages')}, cb);
};
