...
DB.prototype.createHChannelSchema = function(){
    ...
    hChannelSchema.pre('save', function(next){
        ...
        self.cache.hChannels[this.chid] = _.clone(this);
        next();
    });
...
DB.prototype.createHMessageSchema = function(){
    ...
    //Add schema to schema's list
    if(!(this.schemas instanceof Array)) this.schemas = [];
    this.schemas.push({name: 'hMessage', schema: hMessageSchema});
...
DB.prototype.createHCommandSchema = function(){
    ...
    //Add schema to schema's list
    if(!(this.schemas instanceof Array)) this.schemas = [];
    this.schemas.push({name: 'hCommandSchema', schema: hCommandSchema});
...
DB.prototype.createHResultSchema = function(){
    ...
    //Add schema to schema's list
    if(!(this.schemas instanceof Array)) this.schemas = [];
    this.schemas.push({name: 'hResult', schema: hResultSchema});
    ...
    this.models.hChannel.find({}, function(err, docs){
        if(!err){
            ...
            docs.forEach(function(channel){
                self.cache.hChannels[channel.chid] = channel;
            });
