    ...
    this.checkValidity(hMessage, context, function(err, result){
        if(!err){
            ...
            stream.on("data", function(localhMessage) {
                ...
                /* EBR j'aimerai avoir une confirmation du bon fonctionnement de ce test lorsque les filtres seront implémentés */
                if(firstElement && !context.hClient.filterMessage(localhMessage))
                    stream.destroy();
