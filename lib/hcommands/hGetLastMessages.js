...
hGetLastMessages.prototype.exec = function(hCommand, context, cb){
    ...
    context.models.hChannel.findOne({chid: chid}, function(err, doc){
        if(!err){

            if(doc && doc.participants.indexOf(sender) > -1){

                var it = 0;
                while(it < doc.headers.length && doc.headers[it].hKey != 'maxMsgRetrieval') it++;

                if(doc.headers[it])
                    quant = params.quant || doc.headers[it].hValue || quant; //In case header mal format
