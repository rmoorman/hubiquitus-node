...
hCreateUpdateChannel.prototype.exec = function(hCommand, context, cb){
    ...
    if(existingChannel.owner && !existingChannel.owner.match(channel.owner)){
...
//                    params: {chid: channel.chid}}, context, function(status, result){});
    ...
    db.saveHChannel(channel, function(err, result){
        if(!err){
            ...
            err.code == mongoCodes.INVALID_ATTR ? cb(status.INVALID_ATTR, err.msg) : cb(status.MISSING_ATTR, err.msg);
    });
};
