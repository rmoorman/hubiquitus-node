...
hCreateUpdateChannel.prototype.exec = function(hCommand, context, cb){
    ...
    if( channel.chid ){
        ...
        channel.chid = /^[^#].*/.test(channel.chid) ? '#' + channel.chid : channel.chid;
        channel.chid += /@/.test(channel.chid) ? '' : '@' + validators.getDomainJID(hCommand.sender);

        //Test if valid name and if valid domain
        if( !(new RegExp('@' + hAdmin.domain + '$').test(channel.chid)) )
