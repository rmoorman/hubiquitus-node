...
hCreateUpdateChannel.prototype.exec = function(hCommand, context, cb){
    ...
    if( channel.owner && !validators.compareJIDs(hCommand.sender, channel.owner) )
    ...
    if( validators.isChannel(channel.chid) ){

        //Test if valid name and if valid domain
        if( !(new RegExp('(^[^@]*$|@' + validators.getDomainJID(hCommand.sender) + '$)').test(channel.chid)) )
        ...
        channel.chid = validators.normalizeChannel(channel.chid, validators.getDomainJID(hCommand.sender));
    ...
    if(existingChannel)
        for(var i = 0; i < existingChannel.participants.length; i++)
            if(channel['participants'].indexOf(existingChannel.participants[i]) < 0)
                unsubscriber.exec({
                    sender: existingChannel.participants[i],
                    params: {chid: channel.chid}
                }, context, function(status, result){});
    ...
    try{
        useValidators = validators.getBareJID(hCommand.sender) != validators.getBareJID(hAdmin.jid);
    } catch(err){
        useValidators = true;
    }
