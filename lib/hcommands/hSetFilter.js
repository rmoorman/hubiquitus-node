...
hSetFilter.prototype.exec = function(hMessage, context, cb){
    ...
    if(validator.isChannel(hMessage.actor)){
        ...
        db._updater(db.get('hChannels'), {_id: hMessage.actor}, {$set: {filter: cmd.filter}}, {upsert: true}, function(){});
        db.cache.hChannels[hMessage.actor].filter = cmd.filter;
    }
    else{
        //If nothing exists, create it
        if( !context.hClient.filter[hMessage.publisher]){
            context.hClient.filter[hMessage.publisher] = {};
        }

        context.hClient.filter[hMessage.publisher] = cmd.filter;
    }
