    ...
    this.validateCmd(hMessage, context, function(err, result){
        if(!err){
            ...
            stream.on("data", function(localhMessage) {
                /* EBR cette instruction doit être inutile */
                ...
                /* EBR pourquoi ce if ??? on est dans la fonction de parcours, et on a déjà manipulé l'objet
                    ne devrait on pas mettre 
                        if (hMessage)
                    pour prendre en compte le résultat du filtre ?
                */
                if(localhMessage)
