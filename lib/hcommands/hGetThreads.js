...
hGetThreads.prototype.exec = function(hCommand, context, cb){
    ...
    this.checkValidity(hCommand, function(err, result){
        if(!err){
            ...
            hCommand.params.chid = /^[^#].*/.test(hCommand.params.chid) ? '#' + hCommand.params.chid : hCommand.params.chid;
            hCommand.params.chid += /@/.test(hCommand.params.chid) ? '' : '@' + validator.getDomainJID(hCommand.sender);
