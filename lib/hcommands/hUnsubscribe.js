        ...
        if(!err){
            if(doc){
                ...
                if(doc.subs.indexOf(channel) > -1){
                    ...
                    //Test if active channel
                    if(db.cache.hChannels[channel].active == false){
                        log.debug('User trying to unsubscribe from an inactive channel');
                        cb(status.NOT_AUTHORIZED, 'trying to unsubscribe from an inactive channel');
                        return;
                    }
                    ...
                    doc.save(function(err){
                        if(!err){
                            self.XMPPUnsubscribe(channel, user, context, function(err){
                                log.debug('Finished unsubscribing to channel correctly');
                                cb(status.OK);
                            });
                            ...
                            log.debug('Finished unsubscribing to channel with status', statusValue, resultValue);
