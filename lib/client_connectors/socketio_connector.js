...
function connect(client, data){
    ...
    client.hClient.on('connect', function(){
        ...
        client.hClient.on('hResult', function(hResult){
            log.info("Sent result to client " + client.id, hResult);
            client.socket.emit('hResult', hResult);
            client.rid++;
        });

        client.hClient.on('hCommand', function(hCommand){
            log.info("Sent command to client " + client.id, hCommand);
            client.socket.emit('hCommand', hCommand);
            client.rid++;
        });
