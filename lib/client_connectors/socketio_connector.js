...
var CommandController = require('../hcommand_controller.js').Controller;
...
function connect(client, data){
    ...
    client.xmppConnector.on('connection', function(attrs){
        ...
        var cmdOptions = options.commandOptions;
        cmdOptions.jid = client.publisher;
        client.cmdController = new CommandController(cmdOptions);
...
function addSocketListeners(client){
    client.socket.on('hCommand', function(hCommand) {
        ...
        if(hCommand && hCommand.entity == 'hnode@' + client.domain)
            client.cmdController.execCommand(hCommand, function(hResult){
                log.info("Sent message to client " + client.id, hResult);
                client.socket.emit('hResult', hResult);
                client.rid++;
            });
