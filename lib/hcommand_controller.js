...
Controller.prototype.createResult = function(hMessage, status, resObject){
    return this.context.hClient.buildResult(hMessage.publisher, hMessage.msgid, hMessage.payload.cmd, status, resObject);
};
...
Controller.prototype.execCommand = function(hMessage, cb){
    ...
    if(module){
        ...
        var onResult = function(status, result){
            ...
            //Save result
            if( hMessageResult.transient === false ){
            ...
            log.info('hCommand Controller sent hMessage with hResult', hMessageResult);
            cb(hMessageResult);
        };
        ...
        timerObject = setTimeout(function(){
            ...
            hMessageResult = self.createResult(hMessage, status.EXEC_TIMEOUT);
        ...
        //Module not found
        hMessageResult = self.createResult(hMessage, status.NOT_AVAILABLE);
