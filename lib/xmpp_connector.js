...
/**
 * @param params
 * { (String) userid: User's XMPP username in format user@domain
...
var XMPPConnector = function(params){
    ...
    this.parameters.jid = params.userid;
...
XMPPConnector.prototype.establishConnection = function(){
    ...
    this.client.on('error', function(msg){
        if( msg == 'XMPP authentication failure' )
            self.emit('link', {status: statuses.Error, code: errors.AUTH_FAILED});
    });
};
...
XMPPConnector.prototype.connect = function(){
    ...
    this.client.on('online',
        function() {
            ...
            self.emit('connection', {userid: self.parameters.jid});
    ...
    this.client.on('stanza', function(stanza){
        ...
        if(parsedMessage){
            for(var i in parsedMessage.entries)
            ...
            if (req){
                switch(req.action){
                    ...
                    case 'subscriptions':
                        self.emit('error', {
                            type: req.action,
                            code: undefined, //TODO: IMPLEMENT ERROR CODES
                            channel: req.channel,
