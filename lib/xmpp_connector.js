...
XMPPConnector.prototype.establishConnection = function(){
    this.emit('status', statuses.Disconnected);
...
XMPPConnector.prototype.connect = function(){
    ...
    this.client.on('online',
        function() {
            ...
            self.emit('status', statuses.Connected);
            log.info("Presence Sent to server");
        });

    //Wait for subscribed messages to arrive
    xmppParser.on('stanza', function(stanza){
        self.emit('message', {type: 'item', content: stanza});
    });
    this.client.on('stanza', function(stanza){ xmppParser.parseData(stanza); });
};
...
XMPPConnector.prototype.disconnect = function(){
    this.emit('status',statuses.Disconnecting);
 ...
 */
XMPPConnector.prototype.subscribe = function(subsName){
 ...
 */
XMPPConnector.prototype.unsubscribe = function(subsName, subsId){
    var ctx = this;
 ...
 */
XMPPConnector.prototype.publish = function(node, entry){
        ...
        if(stanza.is('iq')){
            ...
            if (stanza.attrs.type === 'result'){
                ...
                if(ctx.openSetReqs[id]){
                    ...
                    ctx.emit('message', {type: ctx.openSetReqs[id].action, content: data});
                ...
                //Get requests are not implemented. We sent them back as error to the server.
    ...
    this.client.on('stanza', function(stanza) {
        if(stanza.is('iq') &&
            ...
            if (req){
                ...
                ctx.emit('message', {type: req.action, content: data});
                delete req;
