...
function subscribe(client, data){
    ...
    //Needs to wait until xmpp.status is connected to execute actions
    if(!client.xmppConnection || (client.xmppConnection && client.xmppConnection.status != xmpp.statuses.connected)){
        log.error('Client ID ' + client.id + ' tried to make a subscription, but the client is not connected. Aborting' );
        return;
    }
...
function unsubscribe(client, data){
    ...
    //Needs to wait until xmpp.status is connected to execute actions
    if(!client.xmppConnection || (client.xmppConnection && client.xmppConnection.status != xmpp.statuses.connected)){
        log.error('Client ID ' + client.id + ' tried to make a unsubscription, but the client is not connected. Aborting' );
        return;
    }
