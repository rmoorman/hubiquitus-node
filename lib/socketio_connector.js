...
exports.startSocketIOConnector = function(opts){
    ...
    var channel = io
        ...
        .on('connection', function (socket) {
            ...
            socket.once('hDisconnect', function(){ disconnect(clients[id], true); });
        });
};
...
function attach(client, data){
    ...
    if( supposedClient &&
        ...
        client.socket.emit('link', {status: status.Error, code: errors.ATTACH_FAILED});
    }
}

/**
 * Disconnects the current session and socket. If force is not specified,
 * the socket is closed but not the XMPP Connection (for reattaching)
 * @param client - Reference to the client to close
 * @param force - If we should close also the XMPP Connection or not
 */
function disconnect(client, force){
    ...
    else if (client && client.xmppConnector){
        log.info('Activating timeout for Client ' + client.id);
        var timeout = options['socket.io.disctimeout'];
        client.discTimeout = setTimeout(disconnectClient, timeout);
    }
    else if (client){
        delete clients[client.id];
    }
