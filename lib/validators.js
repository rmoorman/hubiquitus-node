...
exports.validateHChannel = function(hChannel, cb){
    var i;
    var required = ['type', '_id', 'owner', 'subscribers', 'active'];
    ...
    if(hChannel._id && typeof hChannel._id != 'string')
        return cb(codes.INVALID_ATTR, 'actor is not a string');

    if(!exports.isChannel(hChannel._id))
...
/* EBR TODO faire un test unitaire pour cette fonction */
/**
...
exports.validateFilter = function(hMessage){
    ...
    if(channel !== undefined && Object.getOwnPropertyNames(channel.filter).length > 0){
        for(i = 0; i < channel.filter.in.publisher.length; i++){
            if(!validate && channel.filter.in.publisher[i] === publisher){
                validate = true;
            }
        }
    }
