...
exports.validateHChannel = function(hChannel, cb){
    ...
    if(!exports.validateJID(hChannel._id))
        return cb(codes.INVALID_ATTR, 'chid not valid ' + hChannel._id);

    if(/^#hAdminChannel@/.test(hChannel._id))
        return cb(codes.INVALID_ATTR, 'using reserved keyword ' + hChannel._id + ' as chid');
...
exports.validateHMessage = function(hMessage, cb){
    ...
    if( !hMessage.chid )
        return cb(codes.MISSING_ATTR, 'missing chid attribute in hMessage');

    if( !exports.validateJID(hMessage.chid) )
        return cb(codes.INVALID_ATTR, 'hMessages chid is invalid');
    ...
    if(exports.isChannel(hMessage.chid)){
 ...
 */
exports.normalizeChannel = function(chid, domain){
    return chid ? '#' + chid.match(/^#?([^@\/<>'"]+)/)[1] + '@' + domain : null;
};
